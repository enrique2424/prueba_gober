...
const create = async (req, res) => {
    try {
        ...
        const {
            nombre,apat,amat,direccion,fecha_nac,telefono,email
        } = req.body;
    
        const response = await pool.query('insert into clientes(nombre,apat,amat,direccion,fecha_nac,telefono,email) values($1,$2,$3,$4,$5,$6,$7) RETURNING *;', [nombre,apat,amat,direccion,fecha_nac,telefono,email]);
        res.status(200).json(response.rows);
    } catch (error) {
        console.log(error);
    }
...
const update = async (req, res) => {
    ...
    const texto = updateDicamic('clientes', req.body, 'id');
...
const remove = async (req, res) => {
    try { 
        const response = await pool.query('delete from clientes where id = $1 RETURNING *', [req.params.id]);  
        res.status(200).json(response.rows);
    } catch (error) {
        console.log(error);
    }
