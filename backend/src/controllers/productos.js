...
const create = async (req, res) => {
    try {
        ...
        const response = await pool.query('insert into productos(nombre,precio,stock) values($1,$2,$3) RETURNING *;', [nombre,precio,stock]);
        res.status(200).json(response.rows);
    } catch (error) {
        console.log(error);
    }
...
const update = async (req, res) => {
    ...
    const texto = updateDicamic('productos', req.body, 'id');
...
const remove = async (req, res) => {
    try { 
        const response = await pool.query('delete from productos where id = $1 RETURNING *', [req.params.id]);  
        res.status(200).json(response.rows);
    } catch (error) {
        console.log(error);
    }
